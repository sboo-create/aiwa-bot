...
def build_prompt(st, modules):
    ...
    p.append("Только обычный текст на русском, без markdown, без символов # * |, без длинных тире. Сделай акцент дня заметным, не повторяй формулировки изо дня в день, без приветствий. " + TOV)
    return "\n".join(p)
...
def general_summary(profile, mode, hint=None, usage=None):
    ...
    prompt = (_gen_ctx(profile, mode) + h + "\n\n"
        ...
        "Конкретно, без воды, только русский, без markdown. " + TOV)
