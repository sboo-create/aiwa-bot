...
FMT_TG = ("Форматирование: подзаголовки и ключевые слова можно выделять **двумя звёздочками**; списки — пункты с «• »; "
          "если нужна таблица (например КБЖУ) — оформи моноширинным блоком между ``` и выравнивай пробелами. "
          "Другой разметки нет: без #, без | таблиц, без ссылок в скобках.")
...
def build_prompt(st, modules):
    ...
    p.append("Русский язык, без длинных тире. " + FMT_TG + " Сделай акцент дня заметным, не повторяй формулировки изо дня в день, без приветствий. " + TOV)
    return "\n".join(p)
...
def general_summary(profile, mode, hint=None, usage=None):
    ...
    prompt = (_gen_ctx(profile, mode) + h + "\n\n"
        ...
        "Конкретно, без воды, только русский. " + FMT_TG + " " + TOV)
