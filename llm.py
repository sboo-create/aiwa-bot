...
def _gen_ctx(profile, mode):
    ...
    if mode == "fit":
        # SYSTEM зовёт отмечать месячные и /period — в этом режиме команда
        # выключена, и совет ей воспользоваться ведёт в отказ.
        return (
            "Профиль женщины, которая сознательно не отслеживает цикл. Обращайся "
            "в женском роде. Фазы, дни цикла и прогнозы для неё не рассчитываются: "
            "не привязывай к ним советы и не называй её день цикла. Общие вопросы "
            "про цикл, овуляцию и физиологию объяснять можно. НО НЕ предлагай ей "
            "отмечать месячные и не зови в /period, /addcycles, /calendar и календарь "
            "цикла: в этом режиме они выключены. Сама тему ведения цикла не поднимай — "
            "она уже выбрала режим без него, не возвращайся к этому в сводках и советах. "
            "Только если она прямо спрашивает, как начать вести цикл или отметить месячные, "
            "подскажи режим «Цикл» в профиле приложения или команду /mode. "
            f"Возраст примерно {age} ({band}).{diet}\n{_identity_note(profile)}"
        )
    return (
...
def general_summary(profile, mode, hint=None, usage=None):
    ...
    if mode == "fit":
        prompt = (_gen_ctx(profile, mode) + h + "\n\n"
            ...
            "Не предлагай включить отслеживание цикла и не упоминай, что его можно включить. "
            "Конкретно, без воды, только русский. " + FMT_TG + " " + SUMMARY_LEN + " " + TOV)
        out = _call([{"role": "system", "content": SYSTEM}, {"role": "user", "content": prompt}], max_tokens=1000, temperature=0.3, usage=usage)
        return _ensure_complete(_clean(out, None)) if out else None
    prompt = (_gen_ctx(profile, mode) + h + "\n\n"
...
def classify_journal_event(text, usage=None, context=None, enable_v2=False):
    ...
    v2_actions = (
        ...
        "из самой записи. Дополнительно верни repeat_scope: \"meal\", если речь про весь приём целиком "
        "(«повтори этот обед», «съел то же самое» о приёме из нескольких блюд); \"last_item\", если речь "
        "про последнее блюдо из записи («ещё одну такую же тарелку», «ещё один такой же» сразу после "
        "разговора о конкретном продукте); \"unclear\", если по фразе честно нельзя выбрать между ними. "
        "Не угадывай: \"unclear\" лучше неверной догадки, её уточнит сам бот. Это НЕ repeat_meal, если "
        "блюдо названо словами (тогда food) или если она просит показать дневник, а не записать ещё раз;\n"
        if enable_v2 else ""
    )
    target_rule = (
