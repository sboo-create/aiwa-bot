...
def explain_section(st, key, usage=None):
    ...
    if key == "training":
        q = ("Подробно объясни, какая физическая нагрузка уместна именно в эту под-фазу и обязательно ПОЧЕМУ с точки зрения гормонов "
             ...
             "Дай 3-4 конкретных варианта тренировки под этот день с деталями: тип, интенсивность, длительность; "
        ...
        q = ("Подробно объясни, что есть именно в эту под-фазу и ПОЧЕМУ: какие нутриенты сейчас важны и какой эффект дают. "
             "Дай 4-5 конкретных продуктов строками «• продукт - нутриент и зачем», коротко идею завтрака, обеда и ужина, "
    ...
    out = _call(msgs, max_tokens=450, temperature=0.4, usage=usage)
...
def _static(st):
    ...
    return [("", t) for t in S.get(st["phase"], S["luteal"])]
