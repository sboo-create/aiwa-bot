...
def training_review(workout, recent=None, phase_ru=None, mode=None, profile=None, usage=None):
    ...
    if male:
        ...
        ctx.append("Режим: " + str(mode) + " (цикл не отслеживается, опирайся на самочувствие).")
...
def proactive_compose(topic, data_note, mode=None, usage=None):
    ...
    prompt = (identity + "Составь ОДНО короткое проактивное сообщение "
              ...
              "Сделай личным и конкретным (используй данные), по делу, максимум 300 знаков; ОБЯЗАТЕЛЬНО заверши последнюю мысль, не обрывай фразу. " + TOV + " Заверши мягким приглашением открыть Айву, где это можно сделать прямо сейчас (" + example + "загляни в меню в Айве; включим короткую практику в приложении). НЕ обещай, что сделаешь что-то сама автоматически, и не задавай общих вопросов вроде как самочувствие сегодня и не пиши как тебе такая идея. "
