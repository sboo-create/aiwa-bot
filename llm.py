...
def card_captions(mode, ctx, usage=None):
    ...
    want = {
        "cycle": '{"feature":"особенность фазы, 4-8 слов","food":"питание, одно короткое предложение","activity":"нагрузка, одно короткое предложение"}',
        "preg": '{"feature":"особенность срока, 4-8 слов","food":"питание, одно короткое предложение","activity":"активность, одно короткое предложение"}',
        "meno": '{"focus":"фокус дня, 2-3 слова","theme":"тема самочувствия, 2-3 слова","feature":"что с телом и почему, 4-8 слов","food":"питание, одно короткое предложение","activity":"движение, одно короткое предложение","sleep":"сон или настроение, одно короткое предложение"}',
...
_FOOD_FORMAT = ("Ответь СТРОГО одним JSON-объектом без markdown и без пояснений:\n"
    '{"title":"короткое общее название",'
    '"fclass":"белковое|углеводное|овощи и фрукты|молочное|жиры и орехи|сладкое|напиток|смешанное",'
    '"items":[{"name":"продукт или блюдо","grams":число,"kcal":число,'
    '"protein":число,"fat":число,"carbs":число}],'
    '"unparsed":["фрагменты описания, которые не удалось отнести ни к одному продукту"]}\n'
    "ВАЖНО: каждое названное блюдо или продукт — ОТДЕЛЬНЫЙ элемент items. Ничего не объединяй "
    "и ничего не выбрасывай. Если фрагмент похож на искажение распознавания речи и ты не "
    "уверена, что это за продукт, НЕ придумывай его и НЕ пропускай молча — положи фрагмент "
    "как есть в unparsed. Если сказано, что съедена часть порции (например половина), "
    "уменьши граммы и КБЖУ соответственно. Числа целые, без единиц измерения. "
    "Если вес не назван — поставь реалистичную оценку типичной порции.")
...
def analyze_workout_text(text, usage=None):
    ...
    prompt = (
        ...
        '"weight":число_или_null,"group":""}],"note":"короткие дополнительные факты"}. '
        "Если вид тренировки не указан, но перечислены упражнения, поставь «Силовая». "
