...
def compute_dashboard(days: float = 1.0, source: str = "mixed") -> dict[str, Any]:
    ...
    # Один compute_retention на дашборд: он вычитывает всю таблицу, и карточки
    # ретенции с черновиком канона делят один и тот же результат.
    fleet_retention: dict = {}
    try:
        fleet_retention = compute_retention(now)
    except Exception:  # noqa: BLE001 — карточки деградируют, ядро живёт
        pass
    return {
...
# отваливаются молча — у AIWA вызов модели зовётся ai_call, — а тихо
# завышенные сессии хуже отсутствующих. Список, а не блок-лист: пропущенное
# имя занижает и это видно, лишнее машинное имя завысило бы незаметно.
# Пустой список = карточек сессий у модуля нет, и это честнее выдумки.
# Имена сырые, как в таблице: ALIASES применяется уже в Python, поэтому
# legacy-имена перечислены рядом со своими каноническими.
HUMAN_EVENT_NAMES: tuple[str, ...] = (
    "app_opened", "screen_viewed", "user_message_sent",
    "legacy_button", "legacy_command", "legacy_message_interaction",
    "summary_opened", "push_opened",
    "checkin_completed", "checkin_updated", "checkin_symptom_selected",
    "answer_feedback_submitted", "onboarding_started", "onboarding_completed",
    "meal_add_completed", "workout_add_completed",
    "food_flow_started", "workout_flow_started",
    "legacy_signup", "legacy_activated", "legacy_ref", "legacy_goal",
    "proactive_enabled", "proactive_disabled")
# Намеренно НЕ здесь, хотя по имени выглядят человеческими:
# legacy_stt — вызов провайдера распознавания (calls=1) сразу после
#   голосового, пишется даже когда расшифровка пустая; сам человеческий
#   факт строкой ниже уже уходит как user_message_sent;
# legacy_tts — озвучка ответа, инициатива бота;
# legacy_remember и legacy_memory_learn — инструмент агента (meta="agent")
#   и фоновая выжимка фактов: память пишет модель, а не пользователь.
# legacy_goal здесь есть: все его сохранившиеся meta (report, partner_link,
# period) начинаются с действия человека — send_report зовут только
# callback «rep:» и Mini App /api/report, планировщика у него нет.
# app_opened тоже человеческое: его пишет _api_data_sync, а туда нельзя
# попасть без initData Telegram, то есть без открытого пользователем
# webview. Автостарта, вебхука и серверного вызова у этого пути нет
# (в GigaType такой автостарт завысил сессии в полтора раза).
...
# Здесь только работа модели и агента. Доставка уведомлений
# (push_queued/push_sent/push_failed/push_shadowed, summary_delivered)
# намеренно не входит: планировщик шлёт их каждому пользователю каждый
# день независимо от того, пришёл он или нет, и они бы утопили сигнал
# реальной агентской работы в размере базы.
MACHINE_EVENT_NAMES: tuple[str, ...] = (
    "ai_call", "ai_usage_recorded", "ai_job_status_changed",
    "assistant_message_sent", "assistant_response_received",
    "tool_execution_completed", "tool_outcome_completed", "legacy_ai_usage")
...
def _canonical_cards(now: float, retention: dict | None = None) -> list[dict]:
    ...
    - «gap» — интервалы между human-событиями с разрывом 30 минут (фрейм
      Цева: сессия = когда человек касается ноута);
    ...
    продуктовая карточка Tools / DAU.
    ...
    Карточки считает SQLite: вычитывать таблицу событий в Python нельзя —
    это сотни тысяч строк и +секунды к /summary, из-за которых поллер хаба
    ловит таймаут и красит модуль в degraded.
    """
    cards: list[dict] = []
    try:
        current = datetime.fromtimestamp(now, PRODUCT_TZ)
