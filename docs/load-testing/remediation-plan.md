...
> Update 2026-07-28: staging использует нативный OpenRouter-клиент без
> LiteLLM. Так как OpenRouter/Cloudflare блокирует публичный egress IP i167
> `158.160.163.167` с HTTP 403, HTTPS до OpenRouter временно передаётся через
> ограниченный SSH egress tunnel на i196. LiteLLM в request path не участвует,
> модель не меняется. Диагностика и измерения зафиксированы в
> `reports/load-testing/20260728T0934Z-direct-openrouter/REPORT.md`.
>
> Update 2026-07-28 14:10 MSK: provider-routing A/B без смены модели выбрал
> allowlist `deepinfra,coreweave,akashml,digitalocean,ionstream,morph` с
> `sort=throughput`. Повторный `cold-open-100` дал 100/100 completed и 0
> failed. Эскалация остановлена из-за `MemAvailable < 200 МБ` на общем i196;
> подробности в
> `reports/load-testing/20260728T1100Z-provider-routing/REPORT.md`.
...
После provider-routing A/B staging работает с
`AIWA_LLM_CONCURRENCY=8` и `AIWA_TODAY_CONCURRENCY=6`, сохраняя два общих
слота для чата. Текущая модель не менялась. Проверенный allowlist и
`sort=throughput` устранили финальные empty responses в повторном
`cold-open-100`.

Повышение до общего concurrency 10 / background 8 допускается только после
снятия инфраструктурного stop-guard на i196 или переноса egress: во время
последнего окна `MemAvailable` опускался ниже 60 МБ, хотя LiteLLM не входил в
request path AIWA.
