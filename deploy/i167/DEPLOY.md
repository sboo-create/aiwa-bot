...
5. Отдельным шагом проверяется прямой публичный домен
   `app.aiwa-wellness.app`: он должен отдавать тот же `release_sha`.
...
## Выкатка фронта — только осознанно

`webapp2/assets/deslop` попадает в прод вместе с любым деплоем: workflow
катит дерево целиком. Инцидент 01.08: пересобранный бандл считался
невыкаченным, но уехал со следующим серверным фиксом и уронил экран
«Питание» (ссылка на переименованную переменную).

Правила:

- собранный бандл кладём в `main` только когда готовы его катить;
- изменения `frontend/` проверяем на staging **с открытием всех трёх
  вкладок**, а не только `/health` и API;
- CI-джоб `frontend` (lint + сборка) обязателен: `no-undef` ловит
  переименованные переменные, `rules-of-hooks` — нарушения порядка хуков;
- **каждая пересборка обязана менять версию**: `AIWA_UI_VERSION`
  (имя чанка) и `AIWA_UI_REV` (`?v=rNN` в index.html). Caddy отдаёт
  `/assets` с `Cache-Control: immutable` — при том же имени и той же
  ревизии браузер продолжает исполнять старый файл, и фикс не доезжает
  (проверено на staging 01.08);
- проверка на staging: index.html лежит и в релизе, и в
  `public-current` — обновлять оба, иначе страница тянет старый бандл.
