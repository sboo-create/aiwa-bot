...
/**
 * theme.css is where a colour is allowed to be a literal — it is the token
 * source, and every level of it carries a light/dark pair. Anywhere else in the
 * product a literal is a colour that cannot follow the theme, which is how the
 * whole app once ended up painting black text on Telegram's dark canvas.
 */
...
/**
 * Inline styles are for values only the runtime knows (a percentage, a measured
 * position, a custom property) — or for a token a vendored primitive insists on
 * receiving through `style`, as the tinted Badge does with its own text colour.
 * A static layout or paint value belongs in composition.css, where the rest of
...
for (const file of sourceFiles) {
  ...
  for (const match of source.matchAll(/style=\{\{([\s\S]*?)\}\}/g)) {
    for (const property of match[1].matchAll(STATIC_INLINE_STYLE)) {
      // `var(--…)` is a token, `${…}` is a value only the runtime knows.
