...
const p7 = C.lazy(() => import("./AiwaWebUiChart-aiwa-v185.js?v=r44").then((n) => ({
...
function xp({ isOpen: n, onClose: t, mode: i, revision: l, symptomGroups: o }) {
  const [c, u] = C.useState(!1), [d, p] = C.useState(null), [y, g] = C.useState(!1), [v, b] = C.useState("period"), [j, w] = C.useState({}), T = C.useRef(Promise.resolve()), x = C.useRef(0), A = C.useRef(null), _ = Array.from({ length: 20 }, (I, $) => Ve("getAiwaCalendarMonth", $ - 12)).filter(Boolean), E = !["preg", "meno", "male", "none"].includes(i), M = J_(E ? ["period", "symptoms", "intimacy"] : ["symptoms", "intimacy"]), O = Qh[v] || Qh.symptoms, D = U_(), N = () => {
    g(!1), w({});
  }, V = (I) => {
    b(I), u(!1), g(!0);
  }, H = M.map((I) => ({
...
const J7 = {
  ...
  "status",
  "delay_days"
].every((i) => W7(n, i)) || !Array.isArray(n.periods) || !Array.isArray(n.cycles) || !Array.isArray(n.past_periods) || !n.stats || typeof n.stats != "object" || Array.isArray(n.stats) || ["cycle", "irregular"].includes(n.mode) && !Array.isArray(n.stats.history) ? !1 : n.mode === "preg" ? !!(n.preg && typeof n.preg == "object" && !n.cycle && n.periods.length === 0 && n.cycles.length === 0) : ["meno", "none", "male"].includes(n.mode) ? !n.cycle && n.preg === null && n.periods.length === 0 && n.cycles.length === 0 : ["cycle", "irregular"].includes(n.mode), tR = async (n, t, i) => {
  let l = null;
  try {
...
function aD({ initialMessages: n = [] }) {
  const [t, i] = C.useState(() => n.map((x, A) => ({
    ...
    text: x.text || "",
    suggestions: []
  }))), [l, o] = C.useState(""), [c, u] = C.useState(!1), [d, p] = C.useState(!1), y = dh.useRef(null), g = (typeof window.aiwaData == "function" ? window.aiwaData() : window.aiwaData)?.mode === "male", v = dh.useRef(null);
  C.useEffect(() => {
    t.length || i([{
      id: "hello",
      role: "assistant",
