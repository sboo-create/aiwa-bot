...
def match_intent(t):
    ...
    if re.search(r"(что\s+мне\s+(сегодня\s+|лучше\s+|можно\s+|стоит\s+)*(есть|съесть|поесть|скушать|покушать|кушать|приготовить)|что\s+(можно|лучше|стоит)\s+(съесть|поесть|покушать|скушать|приготовить|есть)|что\s+(съесть|поесть|покушать|скушать|приготовить)|что\s+(мне\s+)?(при)?готовить|какое\s+питани|какая\s+(сегодня\s+)?еда|какие\s+(мне\s+)?продукт|меню\s+(на\s+)?(сегодня|день|завтра)|составь\s+меню|подбери\s+меню|обнови\s+меню|дай\s+меню|покажи\s+меню|чем\s+(мне\s+)?(сегодня\s+)?питат|как\s+(мне\s+)?(лучше\s+)?питат|что\s+по\s+(еде|питани)|(посоветуй|подскажи|дай|хочу|можешь|порекоменду).{0,24}(поесть|съесть|еду|питани|меню|рацион|продукт|блюд)|\bрацион\b|что\s+покушать|еда\s+на\s+сегодня|что\s+поедим|проголодал|что\s+(есть|съесть|приготовить)\s+на\s+(завтрак|обед|ужин|перекус)|что\s+на\s+(завтрак|обед|ужин|перекус))", t): return "food"
