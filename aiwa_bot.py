...
AIWA_VERSION = os.environ.get("AIWA_VERSION", "2026-07-27-v146-summary-dedup")
...
async def log_workout_action(cid, u, text, user_generation=None, mutation_key=None, preparsed_workout=None):
    ...
    prof = profile_of(u) or {}
    # Не придумываем длительность: без неё калорийность нельзя оценить честно.
    rec["kcal"] = (
...
async def run_all():
    ...
    start_event_writer()
    builder = Application.builder().token(os.environ["BOT_TOKEN"]).concurrent_updates(True)
    ...
    site = web.TCPSite(runner, bind_host, port); await site.start()  # nosec B104
