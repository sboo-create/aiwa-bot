...
_WORKOUT_TYPES = {
    ...
    "стретч": "Растяжка", "велосипед": "Кардио", "вело": "Кардио", "скейт": "Кардио",
}
...
_JOURNAL_PUNCT = r"[\s,.!?:;«»\"'—–-]{0,3}"
_JOURNAL_SENTENCE_LEAD_RE = re.compile(
    # Зачин ограничен двумя словами: цепочкой служебных слов нельзя оттеснить
    # настоящее имя вглубь строки («Ну кстати вообще ладно Соня съела творог»).
    r"^" + _JOURNAL_PUNCT
    + r"(?:\b(?:" + _JOURNAL_NON_NAME_STARTERS + r")\b" + _JOURNAL_PUNCT + r"){0,2}$",
    re.I,
)
...
def _journal_third_party_source(text):
    ...
    if re.search(
        ...
        return True
    # Заглавная буква — сигнал имени собственного только в середине фразы.
    # В начале она объясняется началом предложения, и «Пирожок съела»,
    # «Творог съела», «Овсянку съела утром» читались как «имя съело»: запись
    # молча не появлялась (жалоба Сони 08.08.2026, 8 человек за сутки).
...
_JOURNAL_SEMANTIC_CANDIDATE_RE = re.compile(
    ...
    r"бегал\w*|побегал\w*|пробеж\w*|ходил\w*|гулял\w*|плавал\w*|бассейн\w*|"
...
_JOURNAL_WORKOUT_COMPLETED_RE = re.compile(
    r"\b(?:потренир\w*|тренировал\w*|занимал\w*|бегал\w*|побегал\w*|пробежал\w*|"
...
def _journal_workout_named(text, payload):
    """Событие названо: словами модели или словарём активностей.

    Модель часто приводит активность к типу («Кардио», «Силовая»), которого в
    тексте нет. Базовые виды спорта подтверждает словарь разбора чата, а более
    конкретные занятия — каталог. Картинка может отсутствовать: это не должно
    мешать записать «побегал» или «поездил на скейте».
    """
    return bool(
        _journal_workout_grounded(text, payload)
        or _workout_type_from_text(text)
        or SA.RESOLVER.mentions(text)
    )
...
_JOURNAL_WORKOUT_NEGATED_RE = re.compile(
    ...
    r"потренир\w*|тренировал\w*|занимал\w*|бегал\w*|побегал\w*|пробежал\w*|"
    r"ходил\w*|сходил\w*|гулял\w*|плавал\w*|качал\w*|танцевал\w*|"
    r"приседал\w*|поприседал\w*|отжимал\w*|подтягивал\w*|"
...
# «Ещё» продолжает предыдущую запись и в середине зачина: «я ещё то же самое
# съела», «сегодня ещё раз столько же» — живая речь редко начинается прямо с
# «ещё», а жёсткий якорь ^ такие повторы молча терял (жалоба Сони 08.08.2026).
_JOURNAL_CONTEXT_LEAD = (
    r"(?:(?:я|мы|сегодня|потом|затем|утром|днём|днем|вечером|ночью)\s+){0,2}"
)
_JOURNAL_CONTEXT_OPEN_RE = re.compile(
    # Зачин разрешён только перед «ещё»: у «также» и «плюс» он расширил бы
    # продолжение на обычные новые записи вида «я также съела печенье».
    r"^\s*" + _JOURNAL_CONTEXT_LEAD + r"(?:а\s+)?(?:и\s+)?ещ[её]\b|"
    r"^\s*(?:а\s+)?(?:и\s+)?(?:также|плюс)\b|"
...
def _semantic_journal_candidate(text, context=None, enable_v2=False):
    ...
    personal_report_shape = bool(
        ...
        # Форма отчёта: прошедшее время плюс названная еда или активность.
        ...
        # отсекал такие фразы раньше, чем модель успевала их прочитать. Словарь
        # чата идёт до медиакаталога: отсутствие картинки не отменяет событие.
        or (
            len(raw) <= 220
            and _JOURNAL_PAST_ACTION_RE.search(raw)
            and (_journal_workout_named(raw, {}) or FA.RESOLVER.mentions(raw))
        )
    )
