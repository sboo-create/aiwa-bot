...
class KcalFloorTests(unittest.TestCase):
    def test_floor_is_the_basal_metabolic_rate(self):
        self.assertEqual(bot.kcal_floor(166, 40, 21), round(bot.calc_bmr(166, 40, 21)))
        self.assertEqual(bot.kcal_floor(166, 40, 21), 1172)
        # Мужская формула отличается на 166 ккал и тоже должна учитываться.
        self.assertEqual(bot.kcal_floor(181, 82, 41, male=True), 1751)

    def test_floor_never_drops_below_the_previous_lower_bound(self):
        self.assertEqual(bot.kcal_floor(121, 31, 79), 800)
        self.assertEqual(bot.kcal_floor(None, None, None), 800)

    def test_manual_goal_below_basal_rate_is_lifted_on_read(self):
        """Цели ниже базового обмена уже лежат в базе с тех пор, когда границей было 800."""
        profile = {"height": 166, "weight": 40, "age": 21, "activity": 4,
                   "kcal_goal": 800, "male": False}
        self.assertEqual(bot.profile_kcal(profile)[0], 1172)
...
class WebProfileApiTests(_BotCase):
    ...
    def test_goal_saved_with_the_profile_is_lifted_to_the_basal_rate(self):
        bot.upsert(self.cid, mode="cycle", last_period=bot.dtoday().isoformat(),
                   cycle_len=28, height=168, weight=60, age=30)
        ...
        self.assertEqual(response_goal := bot.row(self.cid)["kcal_goal"], 1172)
...
class PrefsGoalFloorTests(_BotCase):
    ...
    def test_starvation_goal_is_lifted_and_explained(self):
        ...
        self.assertEqual(response.status, 200)
        self.assertEqual(payload["kcal_goal"], 1172)
        self.assertEqual(bot.row(self.cid)["kcal_goal"], 1172)
        self.assertIn("1172", payload["kcal_note"])
